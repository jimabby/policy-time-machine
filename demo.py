...
def steps(domain: str, version: str) -> list[tuple]:
    ...
    return [
        ...
        ("Replay coverage",
         "Which cases were replayed, and are the archived inputs still current?",
         ["ptm.provenance", domain, version], False, False),
        ("Archived replay inputs",
         "Keep the facts, policies and judge settings that produced the result.",
         ["ptm.provenance", domain, version, "--snapshots", "-o",
          f"ptm-{domain}-{version}-snapshots.json"], False, False),
        ("Precedent gate",
