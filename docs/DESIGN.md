...
The [README](../README.md) is the tour: what this does, and nine clips of it
doing it. This is the long version — every decision behind those clips, why
each number carries the caveat it carries, and what each of them refuses to
claim. It is the file to read before changing any of it.

- [What each DAG produces](#what-each-dag-produces)
- [Checking the judge](#consistent-is-not-the-same-as-right) — repeatability, a second model, accuracy
- [Reading the change](#does-the-change-land-evenly) — disparity, preflight, sweeps, drafts
- [Running it for a while](#who-takes-out-the-bins) — retention, cache, the gate from a shell
- [The Diff Explorer](#two-views-one-replay)
- [The CLI surface](#no-airflow-no-api-key)
- [Layout, CI and caveats](#layout)
...
`precedent_gate_<domain>` is what the README calls the point, and it was the
...
![The precedent gate failing, and reporting that the candidate policy introduced none of the reversals](gate.gif)
...
## No Airflow, no API key

The whole loop, in about a second:
...
Sixteen steps in about nine seconds, ordered the way the README's tour builds
...
tests/                          1533 tests; the engine's 1352 need nothing but Python
...
- **The DAG tests do not run on Windows, and say so rather than failing.**
  ...
  so a skip there means something has genuinely changed. The engine's 1352 tests,
